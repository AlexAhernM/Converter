        ...
        try:
            ...
            for placemark in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
                ...
                if geometry is not None:
                    ...
                    for coord in coords:
                        values = coord.split(',')
                        if len(values) == 3:  # longitud, latitud, altitud
                            longitud = float(values[0])
                            latitud = float(values[1])
                            altitud = float(values[2])
                            coordenadas.append((longitud, latitud, altitud))
            resultado.set("Coordenadas:")
            texto_resultado.delete('1.0', tk.END)
            for coord in coordenadas:
                texto_resultado.insert(tk.END, f"Longitud: {coord[0]}, Latitud: {coord[1]}, Altitud: {coord[2]}\n")
