...
class CheckboxFrame(customtkinter.CTkFrame):
    def __init__(self, master, values, enabled=True, width=200):
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.selectfile_frame.grid(row=0, column = 0, padx=10, pady=10)
        ...
        self.selectdata_boton.grid(row=0, column=0, padx=10, pady=(25,10), sticky = 'e')
        
        self.selectdata_entry = customtkinter.CTkEntry(self.selectfile_frame, width=290, height=25, corner_radius=6 )
        self.selectdata_entry.grid(row =0, column =1, padx=10, pady=(20,10 ))
        ...
        self.boton_lookmap.grid(row=2, column=1, padx=10, pady=(10,30))
        
        
        # FRAME: ROW 0 - SAVE LABELS 
        self.frame_save_label = customtkinter.CTkFrame(self, width=600, height=160)
        self.frame_save_label.grid (row=0, column=1, padx=10, pady=10)
        self.frame_save_label.grid_propagate(False)
        #self.frame_save_label.grid_rowconfigure(0, weight=1)
        ...
        self.checkbox_type.grid(row=0, column=2, padx=(50,10), pady =10, sticky='nsw')
    
        
        # FRAME: ROW 1 - APPROVAL
        self.approval_frame = customtkinter.CTkFrame(self, height=50)
        ...
        self.preview_frame = customtkinter.CTkFrame(self,  height=400)
...
def select_file(self):
    ...
    for widget in self.approval_frame.winfo_children():
        widget.destroy()
    for widget in self.frame_save_label.winfo_children():
        widget.destroy()
...
def update_preview(self, lat_centro, lon_centro, zoom_start, root, altitud_value):
    ...
    self.mapa_tkinter.grid(row=2, column=0, padx=10, pady=10, sticky='nsew')
    self.mapa_tkinter.rowconfigure(0, weight=1)
...
def show_messages(self, messages):
    ...
    if msg_dxf:
        ...
        self.save_dxf.grid(row=0, column=0, padx=5, pady=(25,5), sticky='we')

    if msg_shp:
        self.save_shp = customtkinter.CTkLabel(self.frame_save_label, text=msg_shp)
        if msg_dxf:
            self.save_shp.grid(row=1, column=0, padx=5, pady=5, sticky='we')
        else:
            self.save_shp.grid(row=0, column=0, padx=5, pady=5, sticky='we')

    if msg_xlx:
        self.save_xlx = customtkinter.CTkLabel(self.frame_save_label, text=msg_xlx)
        if msg_dxf and msg_shp:
            self.save_xlx.grid(row=2, column=0, padx=5, pady=5, sticky='we')
        elif msg_dxf or msg_shp:
            self.save_xlx.grid(row=1, column=0, padx=5, pady=5, sticky='we')
        else:
            self.save_xlx.grid(row=0, column=0, padx=5, pady=5, sticky='we')
