...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    coords_dec = convierte(root, ruta_archivo_kml, obtener_elevacion_valor)
...
def obtener_maximos_minimos(coords, coords_dec):
    ...
    lat_min = min(coords_dec, key=lambda x: x[0])[0]
    lat_max = max(coords_dec, key=lambda x: x[0])[0]
    lon_min = min(coords_dec, key=lambda x: x[1])[1]
    lon_max = max(coords_dec, key=lambda x: x[1])[1]
    ...
    return { 'lat_min': lat_min, 'lat_max': lat_max, 'lon_min': lon_min, 'lon_max': lon_max, 
            'lat_central':lat_centro, 'lon_central': lon_centro }
