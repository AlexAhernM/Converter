...
def transformar_archivo_kml(entrada_archivo_kml):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if ls is not None:
            ...
            msp.add_lwpolyline(utm_points, dxfattribs={'layer': layer_name[0],'color': 7 }) 
                ...
                if linear_ring is not None:
                    ...
                    if coord is not None:
                        ...
                        for point in points:
                            ...
                            coords_shp.append((lat, lon))
                            layers.append(layer_name)
                        msp.add_lwpolyline(utm_points, dxfattribs={'layer': layer_name[0]})  
                        utm_points = [] # limpiar la lista
   ...
   # 1. CAD = DXF
    ...
    data = list(zip([point[0] for point in coords], [point[1] for point in coords], layers))

    # Crear  DataFrame
    df = pd.DataFrame(data, columns=['X', 'Y', 'Capa'])
