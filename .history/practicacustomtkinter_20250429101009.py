...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        #self.grid_columnconfigure(1, weight=0) # La segunda columna ocupará 2/3 parte del ancho
        
        # FRAME : ROW 0 - SELECT FILE
        self.selectfile_frame = customtkinter.CTkFrame(self, width=500)
        self.selectfile_frame.grid(row=0, column = 0, padx=10, pady=10, sticky = 'ns')
        ...
        self.frame_save_label = customtkinter.CTkFrame(self, width=500)
        self.frame_save_label.grid (row=0, column=1, padx=(50,0), pady=10, sitcky ='ns')
        ...
        self.checkbox_type.grid(row=0, column=2, padx=(250,10), pady =10, sticky='nsw')
...
def show_messages(self, messages):
    ...
    if msg_dxf:
        self.save_dxf = customtkinter.CTkLabel(self.frame_save_label, text=msg_dxf, width=350)
        self.save_dxf.grid(row=0, column=0, padx=50, pady=10, sticky='nswe')

    if msg_shp:
        self.save_shp = customtkinter.CTkLabel(self.frame_save_label, text=msg_shp, width=350)
        if msg_dxf:
            self.save_shp.grid(row=1, column=0, padx=50, pady=10, sticky='nswe')
        else:
            self.save_shp.grid(row=0, column=0, padx=50, pady=10, sticky='nswe')

    if msg_xlx:
        self.save_xlx = customtkinter.CTkLabel(self.frame_save_label, text=msg_xlx, width=350)
        if msg_dxf and msg_shp:
            self.save_xlx.grid(row=2, column=0, padx=50, pady=10, sticky='nswe')
        elif msg_dxf or msg_shp:
            self.save_xlx.grid(row=1, column=0, padx=50, pady=10, sticky='nswe')
        else:
            self.save_xlx.grid(row=0, column=0, padx=50, pady=10, sticky='nswe')
