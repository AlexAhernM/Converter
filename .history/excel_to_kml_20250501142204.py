...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.select_boton = ctk.CTkButton(app, text='Select File', command= select_file)
        self.select_boton.grid(row=0, column=0, padx=10, pady=10, sticky ='w')

        self.kml_entry = ctk.CTkEntry(app)
        self.kml_entry.grid(row=0, column=1, padx=10, pady=10, sticky='w')

        self.kml_dec_radio  = ctk.CTkRadioButton(app, text="Coordenadas decimales", variable=self.tipo_coordenadas_var, value="decimales", command=seleccionar_tipo_coordenadas)
        self.kml_dec_radio.grid(row=1, column = 0, padx=10, pady=10, sticky='w')

        self.kml_utm_radio = ctk.CTkRadioButton(app, text="Coordenadas UTM", variable=self.tipo_coordenadas_var, value="utm", command=seleccionar_tipo_coordenadas)
        self.kml_utm_radio.grid(row=1, column = 1, padx=10, pady=10, sticky='w')
        

        self.crear_button = ctk.CTkButton(app, text="Crear archivo KML", command=crear_kml)
