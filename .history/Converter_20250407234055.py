...
def actualizar_imagen_mapa(lat_centro, lon_centro, zoom_start, radio):
    ...
    mapa = Mapa(lat_centro, lon_centro, zoom_start)
    mapa.agregar_datos(lat_centro, lon_centro, radio)
    mapa.guardar_mapa()
    mapa.abrir_mapa()

   # Crear el recuadro para mostrar el mapa
    html_mapa = HTMLLabel(frame_mapa)
    try:
        html_mapa.set_html('<iframe src="file:///D:/OneDrive/tuto_vscode/Conversion/mapa.html" width="800" height="400"></iframe>')
        #html_mapa.set_html('<iframe src="file:///D:/OneDrive/tuto_vscode/Conversion/mapa.html" width="100%" height="100%"></iframe>')
        print("Iframe creado correctamente")
    except Exception as e:
        html_mapa.set_html(f'Error: {str(e)}')
        print(f"Error al crear iframe: {str(e)}")
    html_mapa.pack(fill=tk.BOTH, expand=True)
       
       
frame_mapa = tk.Frame(ventana_principal, bg="red", highlightbackground="black", highlightthickness=1)
frame_mapa.grid(row=5, column=0, columnspan=3)

# funcion que ejecuta el programa
def procesar_archivo():
    _, _, _, lat_centro, lon_centro, zoom_start, radio = parseo(entrada_archivo_kml, obtener_elevacion)
    actualizar_imagen_mapa(lat_centro, lon_centro, zoom_start, radio)
    

# Crear el botón para transformar el archivo
boton_transformar_archivo = tk.Button(ventana_principal, text="Transformar archivo",command=procesar_archivo)
