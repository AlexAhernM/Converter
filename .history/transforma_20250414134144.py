...
def parseo(ruta_archivo_klm, obtener_elevacion):
    ...
    resultado = convierte(root, ruta_archivo_klm, obtener_elevacion_valor)
...
def crear_dxf(doc, ruta_archivo_kml, coords, layers, coords_dec):
   ...
   # 1. CAD = DXF
    ...
    with fiona.open(ruta_archivo_salida_dxf, 'r', driver='DXF') as src:
        ...
        ruta_archivo_salida_shp = ruta_archivo_salida_shp.replace('.shp')
        with fiona.open(ruta_archivo_salida_shp, 'w', driver='ESRI Shapefile', schema={'geometry': 'LineString', 'properties': src.schema['properties']}) as dst_lineas:
            # Agregar los datos del archivo DXF
            for feature in src:
                if feature.geometry.type == 'LineString' or feature.geometry.type == 'Polygon':
                    # Convertir el polígono a línea
