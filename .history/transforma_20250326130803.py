...
def transformar_archivo_kml(entrada_archivo_kml):
   ...
   # Lista de tuplas con coordenadas y nombres
    ...
    for name, coords in zip(names, utm_coords):
        ...
        if not coords:
            print(f"Error: Coordenadas vacías para {name}")
            continue
        
        # Separar las coordenadas del nombre del lugar
        coords = [c for c in coords if isinstance(c, (int, float, np.float64))]
    ...
    # Agregar puntos
    if len(coords[0]) == 3:
        elements.append((name, 'point', (float(coords[0][0]), float(coords[0][1]))))
    ...
    # Agregar polilíneas o polígonos
        ...
        if points[0] != points[-1]:
            elements.append((name, 'lwpolyline', points))
                
        # Agregar polígonos
        else:
            elements.append((name, 'polygon', points))
            
    # Generar archivo DXF
    doc = ezdxf.new('R2000')
    msp = doc.modelspace()
    # Agregar las entidades geométricas al documento DXF
    for name, tipo, coords in elements:
        if tipo == 'point':
            msp.add_point(coords, dxfattribs={'layer': name})
            print (tipo)
        elif tipo == 'lwpolyline':
            msp.add_lwpolyline(coords, dxfattribs={'layer': name[0]})  
            print (tipo)
        elif tipo == 'polygon':
            #msp.add_mpolygon(coords, dxfattribs={'layer': name[0]})
            #msp.add_mpolygon(coords, dxfattribs={'layer': str(name)})
            coords = [(float(c[0]), float(c[1])) for c in coords]
            msp.add_lwpolyline(coords, dxfattribs={'layer': str(name), 'color': 7})  
            print (tipo)
...
def extraer_coordenadas(pm):
    ...
    if ls is not None:
        coord_ls = ls.find('{http://www.opengis.net/kml/2.2}coordinates')
        if coord_ls is not None:
            coords.extend([c.split(',') for c in coord_ls.text.split()])
    ...
    if py is not None:
        ...
        coord_py = linear_ring.find('{http://www.opengis.net/kml/2.2}coordinates')
        if coord_py is not None:
            coords.extend([c.split(',') for c in coord_py.text.split()])
    ...
    if mg is not None:
        ...
        if poly is not None:
            ...
            linear_ring = outer_boundary.find('{http://www.opengis.net/kml/2.2}LinearRing')
            coord_poly = linear_ring.find('{http://www.opengis.net/kml/2.2}coordinates')
            if coord_poly is not None:
                coords.extend([c.split(',') for c in coord_poly.text.split()])
    print (coords)
    return coords, name
