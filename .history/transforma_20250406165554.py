...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    coords = convierte(root, ruta_archivo_kml, obtener_elevacion_valor)
    
    resultados = obtener_maximos_minimos(coords)
    # Acceder a los resultados
    lat_min = resultados['lat_min']
    lat_max = resultados['x_max']
    lon_min = resultados['y_min']
    lon_max = resultados['y_max']
...
def obtener_maximos_minimos(coords):
        
    # Calcula el promedio de las coordenadas x e y
    lat_central = sum(x[0] for x in coords) / len(coords)
    lon_central = sum(x[1] for x in coords) / len(coords)
