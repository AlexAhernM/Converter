...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    coords_utm = convierte(root, ruta_archivo_kml, obtener_elevacion_valor)
    
    resultados = obtener_maximos_minimos(coords_utm)
    ...
    lat_centro = resultados['lat_central']
    lon_centro = resultados['lon_central']
...
def convierte(root, ruta_archivo_kml, obtener_elevacion_valor):
    ...
    coords_utm = []  # Lista de Coordenadas
    doc = ezdxf.new('R2000')
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if mg is None:
            ...
            agregar_polilinea(coord, layer_name, doc,  coords_utm, layers, obtener_elevacion_valor)     
        else:
            coord = mg.find('{http://www.opengis.net/kml/2.2}coordinates')
            geoms = mg.findall('{http://www.opengis.net/kml/2.2}*')
            procesar_multigeometrias(coord, layer_name, doc,  coords_utm, layers, obtener_elevacion_valor, geoms)
            
               
    crear_dxf(ruta_archivo_kml, doc, coords_utm, layers)
    
    
           
    return coords_utm

def obtener_maximos_minimos(coords_utm):
        
    # Calcula el promedio de las coordenadas x e y
    lat_central = sum(x[0] for x in coords_utm) / len(coords_utm)
    lon_central = sum(x[1] for x in coords_utm) / len(coords_utm)

        
    # Obtener los máximos y mínimos de x e y
    lat_min = min(coords_utm, key=lambda x: x[0])[0]
    lat_max = max(coords_utm, key=lambda x: x[0])[0]
    lon_min = min(coords_utm, key=lambda x: x[1])[1]
    lon_max = max(coords_utm, key=lambda x: x[1])[1]
        
    return { 'lat_min': lat_min, 'lat_max': lat_max, 'lon_min': lon_min, 'lon_max': lon_max, 
            'lat_central':lat_central, 'lon_central': lon_central }
                
def procesar_multigeometrias(coord, layer_name, doc,  coords_utm, layers, obtener_elevacion_valor, geoms):
    names = []
    utm_points = []
    elevaciones_api = []
            ...
            if outer_boundary is not None:
                        ...
                        for point in points:
                            ...
                            if obtener_elevacion_valor:
                                ...
                                if respuesta.status_code == 200:
                                    ...
                                    coords_utm.append((utm_point[0], utm_point[1], altitud_api))
                            
                            else:
                                utm_points.append((utm_point[0], utm_point[1],0))
                                coords_utm.append((utm_point[0], utm_point[1],0))
                            
                print(f"Cantidad de elementos multigeometria tipo poligono: {len(coords_utm)}")       
                ...
                if coord is not None:
                        ...
                        for point in points:
                            ...
                            coords_utm.append((utm_point[0], utm_point[1]))
                            layers.append(layer_name)
                            if obtener_elevacion_valor:
                                #Obtener la altitud utilizando la API de Google Maps, generar listas con  coordenadas y capas
                                url = f"https://maps.googleapis.com/maps/api/elevation/json?locations={lat},{lon}&key=AIzaSyACWL7hMm-4fAymx2DY5IkJ5iDlVUy4zEA"
                        ...
                        print (f'Cantidad de elementos tipo linea = {len(coords_utm)}')
...
def agregar_polilinea (coord, layer_name, doc,  coords_utm, layers, obtener_elevacion_valor):
    names = []
    utm_points = []
    elevaciones_api = []
    ...
    print(f"Cantidad de elementos: {len(coords_utm)}")       
    msp.add_polyline3d(utm_points, dxfattribs={'layer': layer_name, 'color': 7})
    utm_points = []  # limpiar la lista
    
    
        
    return coords_utm, layers

    
def crear_dxf(ruta_archivo_kml, doc, coords_utm, layers):
   ...
   # 1. CAD = DXF
    ...
    data = list(zip([point[0] for point in coords_utm], [point[1] for point in coords_utm], [point[2] for point in coords_utm], layers))
