...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if mg is not None:
            procesar_multigeometrias(mg, coords, names, layer_name, layers, msp, ruta_archivo_kml, doc)     
       
    crear_dxf(ruta_archivo_kml, doc, coords, layers)
    return coord, layer_name
                
def procesar_multigeometrias(mg, coords, names, layer_name, layers, msp, ruta_archivo_kml, doc):
