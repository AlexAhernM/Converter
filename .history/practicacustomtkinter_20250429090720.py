...
def show_messages(self, messages):
    ...
    if msg_dxf:
        ...
        self.save_dxf.grid(row=0, column=0, padx=50, pady=10, sticky='we')

    if msg_shp:
        self.save_shp = customtkinter.CTkLabel(self.frame_save_label, text=msg_shp, width=350)
        if msg_dxf:
            self.save_shp.grid(row=1, column=0, padx=50, pady=10, sticky='we')
        else:
            self.save_shp.grid(row=0, column=0, padx=50, pady=10, sticky='we')

    if msg_xlx:
        self.save_xlx = customtkinter.CTkLabel(self.frame_save_label, text=msg_xlx, width=350)
        if msg_dxf and msg_shp:
            self.save_xlx.grid(row=2, column=0, padx=50, pady=10, sticky='we')
        elif msg_dxf or msg_shp:
            self.save_xlx.grid(row=1, column=0, padx=50, pady=10, sticky='we')
        else:
            self.save_xlx.grid(row=0, column=0, padx=50, pady=10, sticky='we')
