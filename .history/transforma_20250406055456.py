...
class Mapa:
    ...
    def agregar_datos(self, lat_centro, lon_centro, radio):
        folium.Circle([lat_centro, lon_centro], radius=radio*20, color='blue').add_to(self.mapa)
...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    try:
        ...
        lat_max_utm, lon_max_utm = utm.from_latlon(lat_max, lon_max)
        lat_min_utm, lon_min_utm = utm.from_latlon(lat_min, lon_min)
        radio = math.sqrt((max(lat_max_utm, lon_max_utm) - min(lat_min_utm, lon_min_utm))**2)
