...
def transformar_archivo_kml(entrada_archivo_kml):
   ...
   # Generar archivo DXF
    ...
    for name, coords in zip(names, utm_coords):
        # Agregar puntos
        if len(coords) == 1:
            elements.append((name, 'point', coords[0][:2]))
        
        # Agregar polilíneas
        elif len(coords) > 1 and coords[0] != coords[-1]:
            elements.append((name, 'lwpolyline', [c[:2] for c in coords]))
        
        # Agregar polígonos
        elif len(coords) > 1 and coords[0] == coords[-1]:
            elements.append((name, 'polygon', [c[:2] for c in coords]))
