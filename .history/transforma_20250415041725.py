...
def procesar_multigeometrias(geoms, coords_dec):
    for geom in geoms:
    ...
    #print (f" capa en multigeometria {layer_name}")
    #print(f"utm_points multigeometrias  tiene {len(utm_points)} elementos")
    #print(f"layers en multigoemetrias tiene  {len(layers)} elementos")   
...
def convierte(root,  obtener_elevacion_valor):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if mg is not None:
            coord, coords_dec = procesar_multigeometrias(mg, coords_dec)
