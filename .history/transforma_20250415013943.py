...
from Converter import actualizar_imagen_mapa
...
def procesar_multigeometrias( geoms, layer_name,  obtener_elevacion_valor):
    for geom in geoms:
    ...
    return coords, coords_dec, layers, utm_points

def convierte(ruta_archivo_kml, root,  obtener_elevacion_valor):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if ls is not None:
            ...
            utm_points, coords, coords_dec, layers= procesar_multigeometrias(mg, layer_name, obtener_elevacion_valor)
        ...
        print (radio)           
        print (f" convierte {layer_name}")
        print(f"utm_points convierte tiene {len(utm_points)} elementos")
        print(f"layers convierte tiene {len(layers)} elementos")       
    ...
    return doc,   layer_name, utm_points, coords, coords_dec, layers, lat_centro,lon_centro, radio
...
def agregar_polilinea(utm_points, layer_name, doc, radio):
    ...
    if len(utm_points) == 1:  # Si es un punto
        ...
        radius = radio  # Radio del círculo en metros
