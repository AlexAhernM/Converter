...
etiqueta_progreso  = customtkinter.CTkLabel(ventana, text="Loading....", font=("Arial", 12),fg_color='transparent')
...
def seleccionar_archivo_kml():
    ...
    entrada_archivo_kml.delete(0, customtkinter.END)
    entrada_archivo_kml.insert(customtkinter.END, ruta_archivo_kml)
    ...
    for placemark in placemarks:
        ...
        if len(coords_dec) == 1:  # Point
            marker = mapa_tkinter.set_marker(coords_dec[0][0], coords_dec[0][1], text=layer_name,
                                            font=("Times New Roman", 8, "bold"), text_color="blue")# icon= "D:\OneDrive\tuto_vscode\Conversion\Posicion.png")
            #marker.change_icon("D:\OneDrive\tuto_vscode\Conversion\Posicion.png")
            mapa_tkinter.set_marker(coords_dec[0][0], coords_dec[0][1])
            #marker.set_text(layer_name)
...
def confirmar_localizacion(doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    etiqueta_confirmacion = customtkinter.CTkLabel(ventana_confirmacion, text="¿La localización es correcta?")
