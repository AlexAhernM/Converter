...
def procesar_placemark(root, ruta_archivo_kml):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        # Procesar líneas
    procesar_geometrias(pm, doc, msp, layer_name, ruta_archivo_kml)
...
def procesar_geometrias(pm, doc, msp, layer_name, ruta_archivo_kml):
    ...
    agregar_polilinea(coord,layer_name, msp, ruta_archivo_kml)
    
    return (coord, layer_name)           
                
def agregar_polilinea (coord,layer_name, msp, ruta_archivo_kml):
    names = []
    utm_points = []
    layers = []
    coords = []
...
def crear_dxf(doc, ruta_archivo_kml):
    import os
   # 1. CAD = DXF
    ...
    nombre_archivo_salida = os.path.splitext(os.path.basename(ruta_archivo_kml))[0] + '_utm.dxf'
    #nombre_archivo_salida = ruta_archivo_kml.split('/')[-1].split('.')[0] + '_utm.dxf'
