...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        if self.__class__.imagen_tk is None:
            # Descargar la imagen y asignarla a App.imagen_tk
            url = "https://raw.githubusercontent.com/AlexAhernM/Converter/main/earth.png"
            response = requests.get(url)
            imagen = Image.open(BytesIO(response.content))
            self.__class__.imagen_tk = ImageTk.PhotoImage(imagen)
    
        # Crear el label de la imagen y asignarle la imagen descargada
    
        self.label_imagen = customtkinter.CTkLabel(self.preview_frame, image=App.imagen_tk, text="")
        self.label_imagen.image = App.imagen_tk
