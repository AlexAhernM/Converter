...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    try:
        ...
        for elem in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark/{http://www.opengis.net/kml/2.2}coordinates'):
            coords = elem.text.split(',')
            lat  = float(coords[1])
            lon = float(coords[0])
            utm_coords = utm.from_latlon(lat, lon)
            print(f"Latitud: {lat}, Longitud: {lon} -> UTM: {utm_coords[0]}, {utm_coords[1]}")
                
            if lat < lat_min or lat_min is None:
                    lat_min = lat
            if lat > lat_max or lat_max is None:
                    lat_max = lat
                    
            if lon < lon_min or lon_min is None:
                    lon_min = lon
            if lon > lon_max or lon_max is None:
                    lon_max = lon
        ...
        lat_centro = sum_lat / count
        lon_centro = sum_lon / count
        
        def convertir_a_utm(lat, lon):
            utm_coords = utm.from_latlon(lat, lon)
            return utm_coords[0], utm_coords[1]
        
        lat_max_utm, lon_max_utm = convertir_a_utm(lat_max, lon_max)
        lat_min_utm, lon_min_utm = convertir_a_utm(lat_min, lon_min)
        
        radio = max(abs(lat_max_utm - lat_min_utm), abs(lon_max_utm - lon_min_utm))        
        
        print("Latitud Centro =" , lat_centro,  "Longitu Centtro ", lon_centro)
        print ("Lat Max UTM = ", lat_max_utm, "Lat Min UTM = ", lat_min_utm,  "Lon Max UTM = ", lon_max_utm, " Lon Min UTM = ", lon_min_utm)
