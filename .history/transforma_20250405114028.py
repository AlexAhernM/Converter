...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    try:
        ...
        # Crear un mapa
        mapa = folium.Map(location=[k.features()[0].geometry.y, k.features()[0].geometry.x], zoom_start=13)

        # Agregar los datos geográficos al mapa
        for feature in k.features():
            folium.GeoJson(feature.geometry()).add_to(mapa)

        downloads_folder = os.path.join(os.path.expanduser('~'), 'Downloads')
        mapa.save(os.path.join(downloads_folder, 'mapa.png'))
