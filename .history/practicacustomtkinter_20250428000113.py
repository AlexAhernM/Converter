...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.checkbox_type = CheckboxFrame(self, values=['DXF (CAD)', 'Shapefile', 'xlxs (Excel)'], state='disabled')
        self.checkbox_type.grid(row=0, column=1, padx=(550,10), pady =10, sticky='nsw')
        
        self.boton_generate_files = customtkinter.CTkButton(self.checkbox_type, text='generate files', state='disabled',
                                                            command=crear_dxf(doc, ruta_archivo_kml, coords, layers, coords_dec))
