...
def uniformar_estilo (ruta_archivo_kml):

    # Cargar el archivo KML
    tree = ET.parse(ruta_archivo_kml)
    root = tree.getroot()

    # Definir los estilos que se van a utilizar
    style_id = 'msn_uniforme'
    style_url = '#' + style_id

    # Crear el estilo uniforme
    style = ET.SubElement(root, 'Style', id=style_id)
    ...
    # Crear el StyleMap
    style_map = ET.SubElement(root, 'StyleMap', id=style_id)
    ...
    style_url_normal.text = style_url
    ...
    style_url_highlight.text = style_url

    # Actualizar los estilos de los Placemarks
    for placemark in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        style_url_element = placemark.find('{http://www.opengis.net/kml/2.2}styleUrl')
        if style_url_element is not None:
            style_url_element.text = style_url

    # Guardar el archivo KML modificado
    ruta_archivo_uniforme = ruta_archivo_kml.replace('.kml', '_uniforme.kml')
    tree.write(ruta_archivo_uniforme)

    return ruta_archivo_uniforme

def parseo(ruta_archivo_uniforme, obtener_elevacion):
    obtener_elevacion_valor = obtener_elevacion.get()
    
    try:
        tree = ET.parse(ruta_archivo_uniforme)
        root = tree.getroot()
    ...
    resultado = convierte(root, ruta_archivo_uniforme, obtener_elevacion_valor)
    
    doc, coords, layers, coords_dec, layer_name = resultado
    ...
    return  coords, coords_dec, lat_centro, lon_centro, zoom_start, radio, doc, layers
...
def convierte(root, ruta_archivo_kml, obtener_elevacion_valor):
    ...
    #return coords, coords_dec doc, coords, layers, coords_dec
    return doc, coords, layers, coords_dec,layer_name
...
def crear_dxf(doc, archivo_uniforme_kml, coords, layers, coords_dec):
   ...
   # 1. CAD = DXF
    ...
    x_coords = [point[0] for layer in coords for point in layer]
    y_coords = [point[1] for layer in coords for point in layer]
    ...
    nombre_archivo_salida_dxf = archivo_uniforme_kml.split('/')[-1].split('.')[0] + '_utm.dxf'
    ...
    # 2. SHP
    nombre_archivo_salida_shp = archivo_uniforme_kml.split('/')[-1].split('.')[0] + '_utm.shp'
    ...
    nombre_archivo_salida = f"{archivo_uniforme_kml.split('/')[-1].split('.')[0]}_utm.xlsx"
