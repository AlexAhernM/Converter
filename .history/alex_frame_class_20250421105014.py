...
class MyCheckboxFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master, values):
        ...
        for i, valores in enumerate(self.values):
            pichulator = customtkinter.CTkCheckBox(self, **valores)
            pichulator.grid(row=i, column=0, padx=10, pady=(10, 0), sticky="w")
            self.checkboxes.append(pichulator)

    def get(self):
        checked_checkboxes = []
        for pichulator in self.checkboxes:
            if pichulator.get() == 1:
                checked_checkboxes.append(pichulator.cget("text"))
