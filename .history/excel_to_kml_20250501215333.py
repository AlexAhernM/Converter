...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.kml_puntos= ctk.CTkRadioButton(self.tipo_frame, text="Puntos", variable=self.tipo_geo, value="Puntos")
        self.kml_puntos.grid(row=0, column=2, padx=10, pady=10, sticky ='w')

        self.kml_poliln= ctk.CTkRadioButton(self.tipo_frame, text="Polilínea", variable=self.tipo_geo, value="Polilínea")
        ...
        self.hemisferio_listbox = ctk.CTkComboBox(self.tipo_frame, values=["Northern", "Southern"])
        ...
        self.select_boton = ctk.CTkButton(self.tipo_frame, text='Select File', command=lambda: select_file(self))
        self.select_boton.grid(row=0, column=0, padx=10, pady=10, sticky ='w')
        
        self.kml_label = ctk.CTkLabel(self.tipo_frame, text="Ruta del archivo Excel:")
        self.kml_label.grid(row=0, column=1, padx=10, pady=10, sticky='w')
        ...
        self.kml_utm_radio = ctk.CTkRadioButton(self, text="Coordenadas UTM", variable=self.tipo_coords, value="utm")
...
def leer_y_transformar_coordenadas(self):
    ...
    if self.tipo_coords.get() == "decimales":    
        ...
        df['lat'], df['lon'] = transformer.transform(df.iloc[:, 0], df.iloc[:, 1])
...
def crear_kml(self):
    ...
    if self.tipo_geo.get() == "Puntos":
        ...
        for index, row in df.iterrows():
            kml.newpoint(name=str(index), coords=[(row['lat'], row['lon'])])
    elif self.tipo_geo.get() == "Polilínea":
        kml = simplekml.Kml()
        coords = [(row['lat'], row['lon']) for index, row in df.iterrows()]
    ...
    indice = 0
    ruta_archivo_kml = os.path.join(ruta_descargas, nombre_archivo_kml)
    while os.path.exists(ruta_archivo_kml):
        indice += 1
    ...
    print(f"Creado y guardado con éxito en: {ruta_archivo_kml}")
