...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if mg is None:
            ...
            procesar_multigeometrias(coord, mg, coords, utm_points, names, layer_name, layers, msp)
            
    crear_dxf(ruta_archivo_kml, doc, coords, layers)       
    return
                
def procesar_multigeometrias(coord, mg, coords, utm_points, names, layer_name, layers, msp):
