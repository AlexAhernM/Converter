...
def parseo(entrada_archivo_kml):
    ...
    procesar_placemark(root, ruta_archivo_kml)
    print (ruta_archivo_kml)
    return root, ruta_archivo_kml

def procesar_placemark(root, ruta_archivo_kml):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        coords, layers = procesar_geometrias(pm, doc, msp, layer_name, ruta_archivo_kml)
        print("Coords en procesar_placemark:", coords)
        print("Layers en procesar_placemark:", layers)     
    crear_dxf(ruta_archivo_kml, doc)
    crear_excel(coords, layers, ruta_archivo_kml)
    
    return (pm, doc, msp, layer_name, ruta_archivo_kml)

def procesar_geometrias(pm, doc, msp, layer_name, ruta_archivo_kml):
    ...
    coords, layers = agregar_polilinea(coord, layer_name, msp)
    print("Coords en procesar_geometrias:", coords)
    print("Layers en procesar_geometrias:", layers)
    return coords, layers

    
                
def agregar_polilinea (coord,layer_name, msp):
    names = []
    utm_points = []
    layers = []
    coords = []
    ...
    for point in points:
        ...
        # Obtener la altitud utilizando la API de Google Maps
...
def crear_dxf(ruta_archivo_kml, doc):
    import os
...
def crear_excel (coords, layers, ruta_archivo_kml):
    import os
