...
global lat_centro, lon_centro, ruta_archivo_kml
lat_centro = 0.0
lon_centro = 0.0
...
def parseo(entrada_archivo_kml, obtener_elevacion):
    ...
    try:
        ...
        for elem in root.iter():
            if elem.tag.endswith('coordinates'):
                ...
                lat_centro = float(coords[1])
                lon_centro = float(coords[0])
                
        
        with fiona.open(ruta_archivo_kml, driver='KML') as src:
            # Imprimir las capas del archivo KML
            print(src.meta)
        
        
        
        #gdf = gpd.read_file(ruta_archivo_kml, driver='KML')
        #gdf = gpd.read_file('KML:{}'.format(ruta_archivo_kml))
        mapa = folium.Map(location=[lat_centro, lon_centro], zoom_start=15)
        mapa = folium.Map(location=[gdf.centroid.y.mean(), gdf.centroid.x.mean()], zoom_start=15)
    ...
    return root,tree, ruta_archivo_kml, lat_centro, lon_centro
