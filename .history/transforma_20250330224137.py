...
def Iteracion( root):
    ...
    if ls is not None:
        ...
        if coord is not None:
            ...
            # Convertir las coordenadas a UTM y agregar la polilínea al espacio de modelo CAD
            for point in points:
                try:
        ...
        if outer_boundary is not None:
            ...
            if linear_ring is not None:
                ...
                if coord is not None:
                    ...
                    # Extraer el modo de altitud (altitudeMode)
                    ...
                    # Diccionario para mapear los valores de altitudeMode a las acciones correspondientes
                    altitude_actions = {
                        'relativeToGround': lambda z: z,
                        'absolute': lambda z: z,
                        'clampToGround': lambda z: 0
                    }
                    ...
                    for point in points:
                        try:
                            lat, lon, z = float(point[1]), float(point[0]), float(point[2])
                            z = altitude_actions[altitude_mode](z)
                            utm_point = utm.from_latlon(lat, lon)
                            names.append(layer_name)
