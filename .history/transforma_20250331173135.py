...
def procesar_placemark(root, ruta_archivo_kml):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        procesar_geometrias(pm, doc, msp, layer_name,  coords, layers)
         
    crear_dxf(ruta_archivo_kml, doc, coords, layers)
    
    return (pm, doc, msp, layer_name, ruta_archivo_kml)

def procesar_geometrias(pm, doc, msp, layer_name,  coords, layers):
    ...
    agregar_polilinea(coord, layer_name, msp, coords, layers)
    
    return coord, layer_name

    
                
def agregar_polilinea (coord,layer_name, msp, coords, layers):
    names = []
    utm_points = []
    elevaciones_api = []
