...
def parseo(ruta_archivo_klm, obtener_elevacion):
    ...
    resultado = convierte(root, obtener_elevacion_valor)
    
    doc, coords, coords_dec   = resultado
    ...
    return   lat_centro, lon_centro, zoom_start, radio, doc
    
def procesar_geometria(doc, coord, layer_name, obtener_elevacion_valor):
    utm_points, coords, coords_dec, layers= obtener_coordenadas(coord, layer_name,  obtener_elevacion_valor)
...
def procesar_multigeometrias(doc, geoms, layer_name,  obtener_elevacion_valor):
    for geom in geoms:
        if geom.tag == '{http://www.opengis.net/kml/2.2}Polygon':
            ...
            coord = geom.find('{http://www.opengis.net/kml/2.2}coordinates')
            procesar_geometria(doc, coord, layer_name,  obtener_elevacion_valor)
...
def convierte(root,  obtener_elevacion_valor):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if ls is not None:
            ...
            utm_points, coords, coords_dec, layers= procesar_geometria(doc, coord, layer_name, obtener_elevacion_valor)
            ...
            utm_points, coords, coords_dec, layers= procesar_multigeometrias(doc, mg, layer_name, obtener_elevacion_valor)
...
def obtener_coordenadas(coord, layer_name,  obtener_elevacion_valor):
