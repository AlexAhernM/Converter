...
def mostrar_advertencia():
    ...
    if obtener_elevacion.get():
        etiqueta_advertencia.config(text="Advertencia: El proceso de transformación toma mas tiempo,  debido al proceso para la  obtención de elevación.")
    else:
        etiqueta_advertencia.config(text="CHUPA ESTE COGOTITO QUE ESTA BIEN GORDITO")      
...
def actualizar_imagen_mapa():
    ruta_archivo = r"C:\Users\alex_\Downloads\mapa.png"
    if os.path.exists(ruta_archivo):
        imagen_mapa = Image.open(ruta_archivo)
        imagen_mapa_tk = ImageTk.PhotoImage(imagen_mapa)
        label_imagen_mapa.config(image=imagen_mapa_tk)
        label_imagen_mapa.image = imagen_mapa_tk
    else:
        print("El archivo no existe")
    

imagen_mapa_tk = ImageTk.PhotoImage(Image.new('RGB', (400, 400), color = (73, 109, 137)))
