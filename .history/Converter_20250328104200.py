...
def seleccionar_archivo_kml():
    ...
    # Definir src_crs y dst_crs
    src_crs = None
    dst_crs = 'EPSG:4326'
    
    # Verificar que zone tenga un valor válido
    if zone is not None:
        src_crs = f'EPSG:{zone}'
        
    # Verificar que src_crs esté definida
    if src_crs is not None:
        transformer = Transformer.from_crs(src_crs, dst_crs, always_xy=True)
    else:
        print("No se pudo obtener la zona del archivo KML")
            
    # Verificar que zone y hemisphere tengan valores válidos
    if zone is not None and hemisphere is not None:
        src_crs = f'EPSG:{zone}'
        dst_crs = 'EPSG:4326'
        transformer = Transformer.from_crs(src_crs, dst_crs, always_xy=True)
    else:
        print("No se pudo obtener la zona y hemisferio del archivo KML")
