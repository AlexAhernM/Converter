...
def show_messages(self, messages):
    ...
    ruta_dxf = msg_dxf.split("en ")[-1] if msg_dxf else None
    ruta_shp = msg_shp.split("en ")[-1] if msg_shp else None
    ruta_xlx = msg_xlx.split("en ")[-1] if msg_xlx else None

    if msg_dxf:
        self.save_dxf = customtkinter.CTkButton(self.checkbox_type, text=msg_dxf, text_color='firebrick1', fg_color="transparent", hover_color="transparent", border_width=0)
        self.save_dxf.configure(command=lambda ruta=ruta_dxf: self.abrir_archivo(ruta))
        self.save_dxf.grid(row=0, column=1, padx=5, pady=(10,5), sticky='w')

    if msg_shp:
        self.save_shp = customtkinter.CTkButton(self.checkbox_type, text=msg_shp, text_color='blue', fg_color="transparent", hover_color="transparent", border_width=0)
        self.save_shp.configure(command=lambda ruta=ruta_shp: self.abrir_archivo(ruta))
        self.save_shp.grid(row=1, column=1, padx=5, pady=5, sticky='w')

    if msg_xlx:
        self.save_xlx = customtkinter.CTkButton(self.checkbox_type, text=msg_xlx, text_color='PaleGreen4', fg_color="transparent", hover_color="transparent", border_width=0)
        self.save_xlx.configure(command=lambda ruta=ruta_xlx: self.abrir_archivo(ruta))
