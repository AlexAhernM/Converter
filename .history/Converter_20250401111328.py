...
def mostrar_advertencia():
    ...
    etiqueta_advertencia.config(text="Este es un texto diferente")
...
checkbox_obtener_elevacion = tk.Checkbutton(ventana_principal, text="Obtener elevación", variable=obtener_elevacion, 
                                             command= lambda: mostrar_advertencia)
checkbox_obtener_elevacion.grid(row=1, column=1, padx=5, pady=5)


# Crear etiqueta para mostrar mensaje
etiqueta_advertencia = tk.Label(ventana_principal, text="chupa la callampa", wraplength=400)

# Colocar etiqueta en la ventana principal
etiqueta_advertencia.grid(row=3, column=0, columnspan=3, padx=5, pady=5)
