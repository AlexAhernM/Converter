...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.kml_dec_radio  = ctk.CTkRadioButton(self, text="Coordenadas decimales", variable=self.tipo_coords_var, value="decimales", command=seleccionar_tipo_coordenadas)
        self.kml_dec_radio.grid(row=1, column = 0, padx=10, pady=10, sticky='w')

        self.kml_utm_radio = ctk.CTkRadioButton(self, text="Coordenadas UTM", variable=self.tipo_coords_var, value="utm", command=seleccionar_tipo_coordenadas)
...
def seleccionar_tipo_coordenadas(self):
    df = self.leer_y_transformar_coordenadas()
