...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.selectdata_boton.grid(row=0, column=0, padx=10, pady=(10,100), sticky = 'e')
        ...
        self.checkbox_type = CheckboxFrame(self, values=['DXF (CAD)', 'Shapefile', 'xlxs (Excel)'])
        self.checkbox_type.grid(row=0, column=1, padx=(550,10), pady =10, sticky='nsw')
        
        # FRAME: ROW 1 - APPROVAL
        self.approval_frame = customtkinter.CTkFrame(self)
        self.approval_frame_frame.grid(row=1, column =0, padx=10,pady = 0, sticky= 'ew', columnspan=2)
        
        # FRAME: ROW 2 - MAP PREVIEW
        self.preview_frame = customtkinter.CTkFrame(self)
        self.preview_frame.grid(row=2, column=0, padx=10, pady=10, columnspan=2, sticky='nsew')
...
def update_preview(self, lat_centro, lon_centro, zoom_start, root, altitud_value):
    ...
    self.label_mappreview.grid(row=0, column =0, padx= (500), pady= (5))        
...
def confirmar_localizacion(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    #self.label_mappreview.grid(row=0, column =0, padx= (500), pady= (5)) 
    boton_looks_good = tk.Button(self.approval_frame, text="Looks good", command=lambda: respuesta_confirmacion("correcta"))
    boton_looks_good.grid(row =0, column=1, padx =10, pady =10, sticky ="w")
    
    boton_no_good = tk.Button(self.approval_frame, text="Localización incorrecta", command=lambda: respuesta_confirmacion("incorrecta"))
    boton_no_good.grid (row=0, column=2, padx=10, pady=10, sticky='e')
