...
def convierte(root, ruta_archivo_kml, obtener_elevacion_valor):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        if layer_name in layer_names:
            ...
            i = 1
            while f"{layer_name} ({i})" in layer_names:
                i += 1
        ...
        if mg is None:
            ...
            agregar_polilinea_simple(coord, layer_name,  coords, layers, obtener_elevacion_valor, coords_dec)     
        else:
            coord = mg.find('{http://www.opengis.net/kml/2.2}coordinates')
            geoms = mg.findall('{http://www.opengis.net/kml/2.2}*')
            procesar_multigeometrias(coord, layer_name,   coords, layers, obtener_elevacion_valor, geoms, coords_dec)
            
               
    
    crear_dxf(ruta_archivo_kml, coords, layers)
...
def agregar_polilinea(utm_points, layer_name):
    doc = ezdxf.new('R2000')
...
def procesar_multigeometrias(coord, layer_name,   coords, layers, obtener_elevacion_valor, geoms, coords_dec):
    for geom in geoms:
        if geom.tag == '{http://www.opengis.net/kml/2.2}Polygon':
            ...
            if outer_boundary is not None:
                ...
                if linear_ring is not None:
                    ...
                    agregar_polilinea(utm_points, layer_name)
...
def agregar_polilinea_simple(coord, layer_name, doc, coords, layers, obtener_elevacion_valor, coords_dec):
    utm_points, coords, coords_dec, layers = obtener_coordenadas(coord, layer_name, coords, coords_dec, layers, obtener_elevacion_valor)
