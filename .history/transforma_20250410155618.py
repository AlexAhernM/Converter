...
def procesar_geometria(doc, geom, layer_name, coords, layers, obtener_elevacion_valor, coords_dec):
...
def procesar_multigeometrias(doc, layer_name, coords, layers, obtener_elevacion_valor, geoms, coords_dec):
    for geom in geoms:
        coords, coords_dec, layers = procesar_geometria(doc, geom, layer_name, coords, layers, obtener_elevacion_valor, coords_dec)
...
def convierte(root, ruta_archivo_kml, obtener_elevacion_valor):
    ...
    for pm in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        ...
        # ...
            ...
            if ls is not None:
                coords, coords_dec, layers = procesar_geometria(doc, ls, layer_name, coords, layers, obtener_elevacion_valor, coords_dec)
            elif py is not None:
                outer_boundary = py.find('{http://www.opengis.net/kml/2.2}outerBoundaryIs')
                if outer_boundary is not None:
                    linear_ring = outer_boundary.find('{http://www.opengis.net/kml/2.2}LinearRing')
                    if linear_ring is not None:
                        coords, coords_dec, layers = procesar_geometria(doc, linear_ring, layer_name, coords, layers, obtener_elevacion_valor, coords_dec)
            elif mg is not None:
                coords, coords_dec, layers = procesar_multigeometrias(doc, layer_name, coords, layers, obtener_elevacion_valor, mg, coords_dec)
            elif pt is not None:
                coords, coords_dec, layers = procesar_geometria(doc, pt, layer_name, coords, layers, obtener_elevacion_valor, coords_dec) 
