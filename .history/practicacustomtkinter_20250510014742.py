...
def show_image_in_preview(self):
    ...
    self.preview_image = customtkinter.CTkImage(light_image=image, dark_image=image, size=(1250, 530)) # Ajusta el tamaño aquí
...
def select_file(self):
    ...
    for widget in self.preview_frame.winfo_children():            
        widget.destroy()
    self.preview_image = customtkinter.CTkImage(light_image=image, dark_image=image, size=(1250, 530))
    if not self.preview_label:
        self.preview_label = customtkinter.CTkLabel(self.preview_frame, text="", image=self.preview_image)
        self.preview_label.grid(row=0, column=0, padx=(60,100), pady=(0,10), sticky='nsew')
